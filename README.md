...
自研 Agent Loop 引擎驱动 20+ 项原子能力、30 余种内置工具，直接接管你的文件、代码、终端、浏览器、屏幕和桌面应用。
...
UseIO 住在你电脑里，接管的是你整台电脑的工作流。编程只是它能力的一部分 — 它还能解析你的 Office 文档和 PDF、读取你的屏幕帮你排查问题、像真人一样操作桌面应用（Computer Use）、管理你的文件和资料、操作浏览器采集数据，还能预设指令定时自动执行例行任务。所有数据 100% 本地处理，零上传；不绑定任何模型，自由配置；记忆持久沉淀，越用越懂你。
...
**20+ 项原子能力 + 30 余种内置工具**，覆盖文件操作、代码执行、命令行、进程管理、浏览器自动化、屏幕读取、文档解析等全方位场景。原子能力可自由组合，灵活应对从简单问答到复杂多步任务的任意场景。工具按安全级别三级管理：低风险静默执行，中风险需确认，高风险始终需确认。
...
一个任务跑起来之后，你不必干等。UseIO 支持**多会话任务**：任务执行期间可以切换到其他会话开启新任务，原任务转入**后台**继续运行，两边互不阻塞，完成后回到对应会话即可查看结果。

考虑到并发压力和电脑负荷，同时运行的任务**最多 3 个**。同时运行的任务越多，对 CPU、内存等资源的占用也越高 —— 需要集中处理复杂任务时，建议适当减少后台任务数量。

> **场景举例**：一个会话里让 Agent 跑构建，另开一个会话改文档，再开一个查资料 —— 三条线并行推进，不用排队等前一个跑完。

### ⏰ 自动化任务 — 预设指令，到点自动执行

把重复性的例行工作交给 UseIO 定时执行：提前写好**预设指令**并设定**触发时机**，到点后 UseIO 会在指定工作空间中自动执行指令，全程无需人工值守。适合每日新闻总结、定时数据整理、周期性报告生成等场景。

- **多种触发方式** — 单次 / 每天 / 每周 / 每月 / Cron 表达式，灵活覆盖各种调度需求
- **独立工作空间** — 每个任务可绑定自己的工作空间，执行时不切换你当前前台空间，互不干扰
- **任务管理** — 支持启用 / 禁用、立即运行、停止运行、编辑、删除，并保留每次触发的运行记录
- **运行状态** — 运行中 / 已完成 / 失败 / 已中止 / 已跳过，执行结果一目了然

> **场景举例**：设定一个「每天 9:00 总结昨日 AI 行业新闻」的自动化任务，到点 UseIO 自动在指定工作空间执行指令、生成 Markdown 报告，你只需早上打开看结果。
...
### 🖱️ Computer Use 桌面自动化 — 像人一样操作你的电脑

看屏幕、点鼠标、敲键盘 — UseIO 的 Computer Use 能力让它像真人一样操作你的电脑：把执行范围从网页扩展到整个操作系统，记事本、Office、系统设置、文件资源管理器，任何桌面应用都可以操控。浏览器自动化负责网页内的操作，桌面自动化则接管浏览器之外的一切。

- **屏幕感知** — 截取屏幕画面并分析，识别界面结构、按钮位置和当前状态
- **窗口管理** — 枚举当前打开的窗口，把目标窗口带到前台
- **鼠标操作** — 单击、双击、右键、拖拽，点按屏幕上的目标位置
- **键盘输入** — 输入文本、模拟按键、组合键（如 Ctrl+C / Ctrl+V），支持中文输入
- **剪贴板** — 读取或写入系统剪贴板，配合组合键实现可靠的文本输入
- **滚动** — 在任意应用窗口内滚动页面

操作遵循「先看后动」的节奏，每一步都走完完整的四步闭环：

1. **感知** — 截屏分析当前桌面状态，确认目标应用和界面元素
2. **激活** — 将目标窗口带到前台，确保操作落在正确的应用上
3. **操作** — 点击、输入、滚动，完成界面交互
4. **验证** — 再次截屏确认操作生效，未生效则自动调整重试

> **与浏览器自动化的分工**：网页内的点击、表单填写、数据采集交给浏览器自动化（元素级定位，更精准）；记事本、Office、系统设置等桌面应用，以及没有网页界面、只能通过客户端使用的软件，交给桌面自动化。两类能力自动协同 — 网页内优先浏览器自动化，浏览器之外交给桌面自动化，无需你手动指定。

> **场景举例**：对它说「把终端里的报错复制到记事本存一份」→ 它定位记事本窗口、粘贴文本、保存文件，全程像真人在操作。

安全方面，截屏分析、窗口枚举等只读操作无需确认；点击、输入等会改变系统状态的操作，需要你确认后才会执行。连续多轮操作无进展时，UseIO 会自动停下并向你说明，避免盲目重试。

> ⚠️ 屏幕感知依赖视觉模型。使用桌面自动化前，请确保已在模型管理中配置支持视觉理解的模型。
...
> - Model Name: `glm-5.3`
>
> 如需使用屏幕读取 / 图片分析 / 桌面自动化功能，还需额外配置一个支持视觉的模型。
...
| 功能 | 说明 |
|------|------|
| **Agent 工具调用** | 20+ 项原子能力 + 30 余种内置工具，自动拆解任务链逐步完成 |
| **多会话任务** | 支持后台并行执行任务，最多 3 个任务同时进行，前台不被阻塞 |
| **自动化任务** | 预设指令 + 触发时机，到点自动执行例行任务，支持单次 / 每天 / 每周 / 每月 / Cron |
...
| **Computer Use 桌面自动化** | 像人一样操作任意桌面应用：屏幕感知、窗口管理、鼠标键盘输入、剪贴板与滚动 |
